        ...
        <el-form label-width="0" ref="dataForm" :model="dataForm" :rules="dataRule">
          ...
          <table class="gridtable center" border>
            <tbody>
              <tr>
                <th width="50%">调研日期</th>
                <td>{{ record.research_date}}</td>
              </tr>
              <tr>
                <th>楼宇序号</th>
                <td>{{ record.house_code}}</td>
              </tr>
              <tr>
                <th>填表人</th>
    ...
    methods: {
      init ()
      {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/house/result/select`),
              method: 'get',
      ...
      loadRecordList () {
        this.$http({
          url: this.$http.adornUrl(`/house/record/data`),
          method: 'get',
          params: this.$http.adornParams({
            'house_id': this.dataForm.id
          })
          ...
          if (data && data.status === 200) {
            this.record.research_date = data.data.research_date
            this.record.house_code = data.data.house_code
            this.record.recorder = data.data.recorder
            this.record.mobile = data.data.mobile
            this.record.social_code = data.data.social_code
            this.record.organization_code = data.data.organization_code
          } else {
            this.$message.error(this.$t(data.message))
          }
        })
      ...
      loadQuestionnaireList () {
        this.$http({
          url: this.$http.adornUrl('/questionnaire/view/1'),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {
          if (data && data.status === 200) {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/house/handle`),
              method: 'post',
